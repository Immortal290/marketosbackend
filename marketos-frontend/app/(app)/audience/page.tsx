...
export default function AudiencePage() {
  ...
  const totalAudience = audienceSegments.reduce(
    (sum, seg) => sum + parseFloat(seg.size.replace(",", "")),
    0
  );
    ...
    <div className="flex flex-col gap-8">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 font-mono text-xs text-black/60">
            {audienceSegments.length} active segments • {totalAudience.toLocaleString()} total contacts
          </p>
        </div>
      ...
      <section className="flex flex-col gap-4">
        {audienceSegments.map((segment) => {
          const engagement = engagementConfig[segment.engagement as keyof typeof engagementConfig];
                ...
                <div className="grid grid-cols-2 gap-4 border-t-[2px] border-black pt-4 md:grid-cols-4">
                  ...
                  <div>
                    ...
                    <div className="flex flex-col gap-0.5">
                      {segment.topLocations.slice(0, 2).map((loc) => (
